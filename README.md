...
Denoising Diffusion Probabilistic Models (DDPMs) generate high-quality images but are slow at inference because they require hundreds or thousands of sequential denoising steps. This project reduces that cost and studies the resulting quality-vs-speed.
 ...
 **Parameter-efficient few-step distillation via LoRA.** We freeze the trained DDPM parameters (the teacher model) and train only small "LoRA adapters" injected into the attention layers (the student model) with a **trajectory-matching distillation objective** (a different loss). The student learns to reproduce, in very few steps, what the teacher produces with many DDIM sub-steps, compensating the error that makes naive few-step sampling collapse.

Because only the adapters are trained (a small fraction of the weights) and they can be merged into the base weights at inference, the method adds essentially zero inference overhead while providing large speed increases over the full baseline.
...
### Environment
Single GPU (tested on Kaggle T4). Versions are saved in `results/requirements_pinned.txt`:
...
3. Run all cells. Training resumes from saved checkpoints, if present.
...
### Reproducibility
Fixed seed (`SEED = 42`), `cudnn.deterministic = True`, `torch.use_deterministic_algorithms(True)`, and a seeded `torch.Generator` so that all methods denoise the same initial noise. The config and pinned versions are exported with the results.

---

## 5. Method

Standard DDPM with an attention U-Net is trained with EMA and mixed precision. Fast sampling is executed with deterministic DDIM. Then we distills the DDPM into a few-step student by training only LoRA adapters with a trajectory-matching loss: for a random interval `t → t'` of the student schedule, the teacher refines with several small DDIM sub-steps and the student must match it in one step. Full detailed method is in the report.
...
|---|---|---|---|---|---|---|---|
| DDPM (1k imgs) | 1000 | 67.3* | 6.26 | 0.847 | 0.381 | 0.678 | 1× |
